...
describe('e2e: demo brief through the live pipeline', () => {
  it(
    ...
    async () => {
      ...
      // ---- R2: the planted fabrication is FLAGGED ("no matching opinion") ----
      const fabricated = ledger.find((e) =>
        (e.caseName ?? e.raw).includes(FABRICATED_NAME_TOKEN),
      );
      expect(fabricated, 'fabricated cite must be extracted').toBeDefined();
      expect(fabricated!.status).toBe('FLAGGED');
      expect(fabricated!.note.toLowerCase()).toMatch(/no matching opinion|fabricat/);
