...
- **PDF-paste resilience (false-flag fix):** real briefs pasted from PDF mangle whitespace — line breaks land mid-citation and parallel cites jam together. The old parser anchored on the year parenthetical, so it grabbed the *last* parallel cite (or a garbage cite), e.g. flagging the landmark **Tinker v. Des Moines, 393 U.S. 503** as a fabrication. Rewrote the parser to: normalize whitespace/newlines first; anchor on the **reporter** (known-reporter list + structural fallback) not the year; collapse parallel cites to the official one; and guard nameless fragments ("rev'd on other grounds" → UNVERIFIED, never a false CITE_MISMATCH). Live: Tinker (all mangled variants) → VERIFIED; Miller→Mosley still CITE_MISMATCH. 44 unit tests (incl. paste-mangled cases) + e2e green.
- _Note: `VERIFICATION.md` reflects the last full re-grade; the PDF-paste fix above is covered by tests + e2e but post-dates it._
